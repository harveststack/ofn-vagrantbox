...
Vagrant.configure('2') do |config|
  ...
  config.vm.define 'openfoodnetwork' do |machine|
    ...
    machine.vm.synced_folder '../openfoodnetwork', '/openfoodnetwork', :type => 'nfs'
    machine.vm.synced_folder '../ansible', '/ansible', :type => 'nfs'
    machine.bindfs.bind_folder '//openfoodnetwork', '//openfoodnetwork'
  ...
  if ansible_installed?
    config.vm.provision 'ansible' do |ansible|
      ...
      ansible.sudo = true
      ansible.groups = {
        'application' => %w(openfoodnetwork),
        'vm' => %w(openfoodnetwork),
        'postgresql' => %w(openfoodnetwork),
        'delayed_job' => %w(openfoodnetwork),
